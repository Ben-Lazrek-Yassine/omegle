...
class OmTraffic:
            ...
            if response is not None:
                ...
                if 'application/json' in response.headers.get('Content-Type', ''):
                    ...
                    if "clientID" in response.json() and response.json()["clientID"]:
                        ...
                        self.console.print(f"Response: - {response.json()}")
                        return client_id
                    else:
    ...
    def sendMessage(self, s: requests.Session, clientId: str):
            ...
            random_suffix = ""
            if self.use_emoji: 
                random_emoji = random.choice(emoji_list.all_emoji)
            if self.rand_prefix: 
                random_prefix = ''.join(random.choice(string.ascii_lowercase + string.ascii_uppercase + string.digits) for i in range(3))
            if self.rand_suffix: 
                random_suffix = ''.join(random.choice(string.ascii_lowercase + string.ascii_uppercase + string.digits) for i in range(3))
